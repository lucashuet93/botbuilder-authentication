...
var BotAuthenticationMiddleware = /** @class */ (function () {
    ...
    */
    function BotAuthenticationMiddleware(server, authenticationConfig, baseUrl, tenantId, botId) {
        if (baseUrl === void 0) { baseUrl = '::'; }
        this.server = server;
        this.authenticationConfig = authenticationConfig;
        this.serverType = this.determineServerType(server);
        this.initializeServerMiddleware();
        this.initializeRedirectEndpoints(tenantId, botId);
        this.initializeEnvironmentVariables();
        this.initializePassport(baseUrl, tenantId, botId);
    ...
    //--------------------------------------- SERVER REDIRECTS -------------------------------------------//
    BotAuthenticationMiddleware.prototype.initializeRedirectEndpoints = function (tenantId, botId) {
        var _this = this;
        //create redirect endpoint for login failure 
        this.server.get('/' + tenantId + '/' + botId + '/auth/failure', function (req, res, next) {
            res.json("Authentication Failed");
        });
        //passport providers ultimately redirect here
        this.server.get('/' + tenantId + '/' + botId + '/auth/callback', function (req, res, next) {
    ...
    ;
    //------------------------------------------ PASSPORT INIT ---------------------------------------------//
    BotAuthenticationMiddleware.prototype.initializePassport = function (baseUrl, tenantId, botId) {
        var _this = this;
        if (this.serverType === ServerType_1.ServerType.Restify) {
            //restify servers can fetch the base url immediately
            this.baseUrl = baseUrl;
            this.initializePassportProviders(tenantId, botId);
        }
        else {
            ...
            this.server.use(function (req, res, next) {
                if (!_this.baseUrl) {
                    ...
                    _this.initializePassportProviders(tenantIdm, botId);
                }
                ;
                next();
            });
    ...
    ;
    BotAuthenticationMiddleware.prototype.initializePassportProviders = function (tenantId, botId) {
        var _this = this;
        var passport = this.serverType === ServerType_1.ServerType.Express ? passportExpress : passportRestify;
        var urlPrefix = '/' + tenantId + '/' + botId;
        ...
        if (this.authenticationConfig.facebook) {
            passport.use(new passport_facebook_1.Strategy({
                ...
                callbackURL: this.baseUrl + urlPrefix + "/auth/facebook/callback"
            ...
            this.server.get(urlPrefix + '/auth/facebook', passport.authenticate('facebook', { scope: facebookScope }));
            this.server.get(urlPrefix + '/auth/facebook/callback', passport.authenticate('facebook', { successRedirect: urlPrefix + '/auth/callback', failureRedirect: urlPrefix + '/auth/failure' }));
        }
        ;
        //Twitter
        if (this.authenticationConfig.twitter) {
            passport.use(new passport_twitter_1.Strategy({
                ...
                callbackURL: this.baseUrl + urlPrefix + "/auth/twitter/callback",
            ...
            this.server.get(urlPrefix + '/auth/twitter', passport.authenticate('twitter'));
            this.server.get(urlPrefix + '/auth/twitter/callback', passport.authenticate('twitter', { successRedirect: urlPrefix + '/auth/callback', failureRedirect: urlPrefix + '/auth/failure' }));
        }
        ;
        //Google
        if (this.authenticationConfig.google) {
            passport.use(new passport_google_oauth_1.OAuth2Strategy({
                ...
                callbackURL: this.baseUrl + urlPrefix + "/auth/google/callback"
            }, function (accessToken, refreshToken, profile, done) {
            ...
            this.server.get(urlPrefix + '/auth/google', passport.authenticate('google', { scope: googleScope }));
            this.server.get(urlPrefix + '/auth/google/callback', passport.authenticate('google', { successRedirect: urlPrefix + '/auth/callback', failureRedirect: urlPrefix + '/auth/failure' }));
        }
        ;
        //Azure AD v2
        if (this.authenticationConfig.azureADv1 || this.authenticationConfig.azureADv2) {
            ...
            var options = isV2_1 ?
                {
                    failureRedirect: urlPrefix + '/auth/failure',
            ...
            passport.use(new passportAzure.OIDCStrategy({
                ...
                responseType: 'code',
                responseMode: 'query',
                redirectUrl: this.baseUrl + urlPrefix + "/auth/azureAD/callback",
            ...
            var url_1 = urlPrefix + '/auth/callback';
            this.server.get(urlPrefix + '/auth/azureAD', passport.authenticate('azuread-openidconnect'));
            this.server.get(urlPrefix + '/auth/azureAD/callback', passport.authenticate('azuread-openidconnect', options), function (req, res, next) {
            ...
            ;
            this.server.post(urlPrefix + '/auth/azureAD/callback', passport.authenticate(urlPrefix + 'azuread-openidconnect', options), function (req, res, next) {
                _this.serverType === ServerType_1.ServerType.Express ? res.redirect(url_1, 302) : res.redirect(302, url_1, next);
            });
            ;
        }
        ...
        if (this.authenticationConfig.github) {
            passport.use(new passport_github_1.Strategy({
                ...
                callbackURL: this.baseUrl + urlPrefix + "/auth/github/callback",
            }, function (accessToken, refreshToken, profile, done) {
            ...
            this.server.get(urlPrefix + '/auth/github', passport.authenticate('github', { scope: githubScope }));
            this.server.get(urlPrefix + '/auth/github/callback', passport.authenticate('github', { successRedirect: urlPrefix + '/auth/callback', failureRedirect: urlPrefix + '/auth/failure' }));
        }
        ;
    };
    ;
    ...
    BotAuthenticationMiddleware.prototype.createAuthorizationUris = function () {
        ...
        if (this.authenticationConfig.facebook)
            authorizationUris.push({ provider: BotAuthenticationConfiguration_1.ProviderType.Facebook, authorizationUri: this.baseUrl + urlPrefix + "/auth/facebook" });
        if (this.authenticationConfig.google)
            authorizationUris.push({ provider: BotAuthenticationConfiguration_1.ProviderType.Google, authorizationUri: this.baseUrl + urlPrefix + "/auth/google" });
        if (this.authenticationConfig.twitter)
            authorizationUris.push({ provider: BotAuthenticationConfiguration_1.ProviderType.Twitter, authorizationUri: this.baseUrl + urlPrefix + "/auth/twitter" });
        if (this.authenticationConfig.github)
            authorizationUris.push({ provider: BotAuthenticationConfiguration_1.ProviderType.Github, authorizationUri: this.baseUrl + urlPrefix + "/auth/github" });
        if (this.authenticationConfig.azureADv1 || this.authenticationConfig.azureADv2) {
            ...
            authorizationUris.push({ provider: azureADProvider, authorizationUri: this.baseUrl + urlPrefix + "/auth/azureAD" });
        }
